...
**要实现接口成员，实现类的相应成员必须是public、非静态的，并且与接口成员具有相同的名称和签名。**
...
一般情况下，使用的都是隐式接口实现，但也会有一些特殊情况需要使用显示接口实现，如果一个[类](https://docs.microsoft.com/zh-cn/dotnet/csharp/language-reference/keywords/class)实现的两个接口包含签名相同（签名由方法名、类型参数成员以及形参的类型与种类构成）的成员，则在该类上实现此成员会导致这两个接口将此成员用作其实现。 如下示例中，所有对 `Paint` 的调用皆调用同一方法。 第一个示例定义类型：
