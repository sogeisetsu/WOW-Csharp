...
之前曾经说到过抽象类有单继承局限，那么如何让一个类派生自多个基类呢？现在有一个方法就是使用接口。接口包含非抽象[`类`](https://docs.microsoft.com/zh-cn/dotnet/csharp/language-reference/keywords/class)或[`结构`](https://docs.microsoft.com/zh-cn/dotnet/csharp/language-reference/builtin-types/struct)必须实现的一组相关功能的定义。接口可以定义`static`方法，这些方法必须在接口中有主体。**从 C# 8.0 开始，接口可以为成员定义默认实现（default implementation）。**接口不能声明**实例数据（有的地方叫做非静态数据或者普通数据，MSDN称之为instance data）**，例如**字段、自动实现的属性或类似属性的事件**。
...
**要实现接口成员，实现类的相应成员必须是public、非静态的，并且与接口成员具有相同的名称和签名。**如果采取在实例化实现类的时候采用的声明对象为接口，则接口的成员可以不一定是public，只要满足为非静态非私有即可在实现类中进行**没有访问修饰符的实现并在以接口作为声明对象的实现类实例化对象**进行调用，**因为声明对象为接口，调用成员时的可访问性按接口定义访问性来**。也就是说，**如果接口的成员不是public**而是**非私有**和**非静态**，要在**实现类中实现该成员只能在实现的时候没有访问修饰符，且实现类的实例化对象声明为接口才能调用该成员。**
...
以上最主要的就是接口中可以包含常量和静态成员。本质上它们是属于接口的数据，在接口中声明它们与在正常的类中声明无异。正常情况下，假设接口`A`有常量`a`，应通过`A.a`来调用`a`。
